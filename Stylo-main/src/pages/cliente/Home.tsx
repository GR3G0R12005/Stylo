        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-12">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full placeholder:text-theme-secondary placeholder:opacity-50 border border-theme-secondary/20 rounded-2xl sm:rounded-[1.8rem] py-4 sm:py-5 pl-12 sm:pl-14 pr-4 focus:outline-none focus:ring-4 focus:ring-theme-primary/10 font-medium shadow-sm search-input"
            />
          </div>
          <button 
          ...
          <section className="mb-12">
            ...
            <div className="space-y-4">
              {appointments.filter(a => a.status !== 'completed' && a.status !== 'cancelled').map(app => (
                <div key={app.id} className="p-5 sm:p-8 rounded-[1.75rem] sm:rounded-[2.5rem] bg-theme-bg dark:bg-zinc-900 border border-theme-secondary/10 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 group hover:shadow-xl transition-all">
                  ...
                  >
                    ...
                    <div className="p-8">
                      <div className="flex flex-wrap gap-2 mb-4">
                        <div className="text-[10px] font-black uppercase tracking-[0.2em] px-3 py-1 bg-theme-secondary/10 text-theme-secondary rounded-full">
      ...
      <AnimatePresence>
        {selectedShop && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 bg-theme-text/20 backdrop-blur-md flex items-end sm:items-center justify-center sm:p-6"
          >
            <motion.div
              ...
              className="bg-theme-bg w-full max-w-2xl rounded-t-[3.5rem] sm:rounded-[3.5rem] p-10 max-h-[90vh] overflow-y-auto shadow-2xl relative"
            >
              <button 
                onClick={() => {
                  setSelectedShop(null);
